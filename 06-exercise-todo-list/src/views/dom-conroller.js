...
const DOMController = (function () {
  const createMainContainerElement = function () {
    ...
    addNewProject.addEventListener("click", function () {
      const projectModalContent = _getProjectModal().querySelector(
    ...
    addNewTask.addEventListener("click", function () {
      const taskModalContent = _getTaskModal().querySelector(
        "#task-modal-content"
      );
    ...
    projectView.addEventListener("submit", function (event) {
      ...
      if (isProjectBeingNewlyCreated) {
        _addProjectDOMToDOMList(
          _createProjectDOM(Datastore.createProject(projectObject))
        );
      } else {
  ...
  const _createTaskView = function (taskObject) {
    ...
    taskView.addEventListener("submit", function (event) {
      ...
      if (isTaskBeingNewlyCreated) {
        _addTaskDOMToDOMList(_createTaskDOM(Datastore.createTask(taskObject)));
      } else {
  ...
  const loadProjectDOMList = function () {
    Datastore.readAllProjects().forEach((projectObject) => {
      _addProjectDOMToDOMList(_createProjectDOM(projectObject));
    });
  };
  ...
  const loadTaskDOMList = function () {
    Datastore.readAllTasks().forEach((taskObject) => {
      _addTaskDOMToDOMList(_createTaskDOM(taskObject));
    });
    _refreshTaskDOMList();
  };

  const _refreshTaskDOMList = function () {
    const showPendingTasksButton = document.querySelector(
      "#show-pending-tasks-button"
    );
    const showPendingTasks = showPendingTasksButton.classList.contains(
      "active-tasks-filter"
    );
    let pendingTasksCount = 0;

    const showCompletedTasksButton = document.querySelector(
      "#show-completed-tasks-button"
    );
    const showCompletedTasks = showCompletedTasksButton.classList.contains(
      "active-tasks-filter"
    );
